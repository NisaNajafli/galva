...
function(a, b, c, d) {
 ...
 // best products carousel
    ...
    // 1) Первый слайдер: после 1100 — 4 айтема
$('.best-products-carousel.best-products-carousel-1').each(function () {
  const $carousel = $(this);
  const $nav = $carousel.closest('section').find('.custom-nav');
...
// 2) Второй слайдер: максимум 3 айтема после 992
$('.best-products-carousel-2').each(function () {
  const $carousel = $(this);
  const $nav = $carousel.closest('section').find('.custom-nav');
  ...
  $carousel.owlCarousel({
    ...
    //     $('#productModal .carousel-indicators').append('<button type="button" style="background-image:url('+$(this).attr('src')+')" data-bs-target="#carouselProducts" data-bs-slide-to='+e+'></li>');
    //     $('#productModal .carousel-indicators button').first().addClass('active');
    // });
    
    // $('.carousel-product-full').find('img').removeClass('carousel-item-left').addClass('carousel-item');
    // $('.carousel-products').find('.carousel-item').first().addClass('active');
    
    
    // news carousel add zero to indicator
